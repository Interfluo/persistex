...
pub fn show(app: &mut App, ui: &mut egui::Ui, ctx: &egui::Context) {
    ui.add_space(6.0);
    ui.label(egui::RichText::new("Inputs").size(SIZE_HEAD).strong().color(TEXT));
    ui.add_space(4.0);
    ...
    egui::Grid::new("input_table")
        ...
        .striped(false)
        .show(ui, |ui| {
            for title in ["", "Input", "f min", "f max", "Tones", "Peak", "Amplitude",
                          "Spacing", "RPF", "Allocated band", "Optimise", "Remove"] {
            ...
            for index in 0..n_rows {
                ...
                changed |= ui.add_enabled(!running,
                    egui::TextEdit::singleline(&mut row.name).desired_width(W_NAME)).changed();
                changed |= ui.add_enabled(!running,
                    egui::TextEdit::singleline(&mut row.f_min).desired_width(W_NUM)).changed();
                changed |= ui.add_enabled(!running,
                    egui::TextEdit::singleline(&mut row.f_max).desired_width(W_NUM)).changed();
                changed |= ui.add_enabled(!running,
                    egui::TextEdit::singleline(&mut row.n_tones).desired_width(W_TONES)).changed();
                changed |= ui.add_enabled(!running,
                    egui::TextEdit::singleline(&mut row.peak).desired_width(W_NUM)).changed();
                ...
                match app.metrics.get(index) {
                    Some((rpf, band)) => {
                        ui.label(egui::RichText::new(format!("{:.4}", rpf))
                            .size(SIZE_BODY).color(TEXT));
    ...
    ui.add_space(6.0);
    ui.horizontal(|ui| {
        if ui.add_enabled(!running, egui::Button::new("Add input")).clicked() {
