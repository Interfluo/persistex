...
fn main() {
    for (label, specs) in [
        ("4x8",  (0..4).map(|i| InputSpec{name:format!("u{i}"),f_min:0.1,f_max:3.0,n_tones:8,..Default::default()}).collect::<Vec<_>>()),
        ...
        println!("{:6} {:.2}s  worst rpf {:.4}", label, t.elapsed().as_secs_f64(), worst);
    }
}
